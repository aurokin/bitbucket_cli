...
const (
	...
	fixtureTempProjectKey     = "BBTMP"
	fixtureTempProjectName    = "bb-cli temp project"
...
func TestBitbucketCloudHumanOutputSmoke(t *testing.T) {
	...
	workspaceViewOutput := session.Run(t, "", "workspace", "view", session.Workspace)
	if !strings.Contains(string(workspaceViewOutput), "Workspace: "+session.Workspace) {
		t.Fatalf("expected workspace label in workspace view output:\n%s", workspaceViewOutput)
	}
	if !strings.Contains(string(workspaceViewOutput), "Next: bb workspace member list "+session.Workspace) {
		t.Fatalf("expected workspace view next step:\n%s", workspaceViewOutput)
	}

	projectViewOutput := session.Run(t, "", "project", "view", fixtureProjectKey, "--workspace", session.Workspace)
	if !strings.Contains(string(projectViewOutput), "Project: "+fixtureProjectKey) {
		t.Fatalf("expected project label in project view output:\n%s", projectViewOutput)
	}
	if !strings.Contains(string(projectViewOutput), "Next: bb project default-reviewer list "+fixtureProjectKey+" --workspace "+session.Workspace) {
		t.Fatalf("expected project view next step:\n%s", projectViewOutput)
	}
...
func deleteProjectIfExists(t *testing.T, client *bitbucket.Client, workspace, key string) {
	t.Helper()

	resp, err := request(t, client, http.MethodGet, fmt.Sprintf("/workspaces/%s/projects/%s", workspace, key), nil)
	if err != nil {
		t.Fatalf("lookup project %s: %v", key, err)
	}
	...
	default:
		...
		t.Fatalf("lookup project %s failed: %s %s", key, resp.Status, strings.TrimSpace(string(body)))
	}

	resp.Body.Close()
	resp, err = request(t, client, http.MethodDelete, fmt.Sprintf("/workspaces/%s/projects/%s", workspace, key), nil)
	if err != nil {
		t.Fatalf("delete project %s: %v", key, err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusNoContent {
		body, _ := io.ReadAll(resp.Body)
		t.Fatalf("delete project %s failed: %s %s", key, resp.Status, strings.TrimSpace(string(body)))
	}
}
