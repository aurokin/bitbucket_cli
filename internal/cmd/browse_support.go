...
var currentBrowseBranch func(context.Context, string) (string, error) = gitrepo.CurrentBranch
...
func buildBrowsePathURL(base, ref, path string, line int) string {
	...
	if strings.TrimSpace(path) == "" {
		rawURL = fmt.Sprintf("%s/src/%s/", base, escapeURLPath(ref))
	} else {
		rawURL = fmt.Sprintf("%s/src/%s/%s", base, escapeURLPath(ref), escapePathSegments(path))
	}
	if line > 0 {
		rawURL += "#lines-" + strconv.Itoa(line)
	}
	return rawURL
}
