...
- Keep behavior grounded in the official Bitbucket Cloud REST API.
- Push after every commit.
- Keep live Bitbucket integration tests manual-only.
- Reuse existing fixture repos when they already exist.
- Update docs when command behavior, flags, output, payloads, or recovery guidance change.
- Regenerate generated docs with `go run ./cmd/gen-docs` when help, flags, payloads, or recovery guidance change.
- Run `go test ./...`.
- When changing human-readable output, add regression tests for field order and `Next:` guidance.
- When changing URL or entity resolution, add regression coverage for messy URLs and canonical URL behavior.
